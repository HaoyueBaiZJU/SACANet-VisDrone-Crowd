...
if __name__=="__main__":
    root = '/media/whitney/datafile/PhD_firstyear_spring/ICCV_workshop/VisDrone/new_separate/'
    ...
    for img_path in img_paths:
        ...
        if kn > 10:
            ...
            if img.shape[1] < 1024:
                ds_rows=int(img.shape[0]//16)
                ds_cols=int(img.shape[1]//16)
                img = cv2.resize(img,(ds_cols*16,ds_rows*16))
                k = cv2.resize(k,(ds_cols*16,ds_rows*16))
           
            else:
                ds_rows=int(img.shape[0]//32)
                ds_cols=int(img.shape[1]//32)
                img = cv2.resize(img,(ds_cols*16,ds_rows*16))
                k = cv2.resize(k,(ds_cols*16,ds_rows*16))
                k = k[np.newaxis,:,:]*2*2

            np.save(img_path.replace('.jpg','.npy').replace('images','rf_GT_vehicle'), k)
            misc.imsave(img_path.replace('images','rf_image_vehicle'), img)
