...
class GraphConvolution(Module):
    ...
    def forward(self, input, adj):
        ...
        B, _, _ = input.shape
        weightfull = torch.unsqueeze(self.weight, 0)
        weightfull = weightfull.expand(B, -1, -1)
        support = torch.bmm(input, weightfull) #torch.mm
        adjfull = torch.unsqueeze(adj, 0)
        adjfull = adjfull.expand(B, -1, -1)
        #output = torch.spmm(adjfull, support)
        output = torch.bmm(support, adjfull)
        if self.bias is not None:
